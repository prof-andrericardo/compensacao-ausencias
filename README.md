# 📚 Atividades de Compensação de Ausências - Curso Técnico em Informática

Este repositório contém atividades de compensação de ausências para o curso Técnico em Informática, organizadas por Unidades Curriculares (UCs).

## 🎯 Estrutura do Repositório

Cada UC possui sua própria pasta com atividades teóricas e práticas específicas:

### 📂 UC5 - Redes de Computadores
- Fundamentos de redes
- Tipos de redes
- Protocolos
- Ferramentas: Cisco Packet Tracer
- [Link para a pasta](#-uc5---redes-de-computadores)

### 📂 UC6 - Planejamento e Instalação de Redes
- Infraestrutura de rede
- Cabeamento estruturado
- Configurações
- Ferramentas: Cisco Packet Tracer
- [Link para a pasta](./UC6)

### 📂 UC7 - Sistemas Operacionais de Redes
- Linux (Debian)
- Serviços de Rede (DNS, Web)
- Administração básica
- Ferramentas: Hyper-V
- [Link para a pasta](./UC7)

### 📂 UC8 - Segurança de Dados e Redes
- Fundamentos de segurança
- Proteção de dados
- Implementação de segurança
- Ferramentas: Packet Tracer, Hyper-V
- [Link para a pasta](./UC8)

### 📂 UC9 - Projeto Integrador
- Desenvolvimento completo
- Documentação
- Apresentação
- Ferramentas: Todas das UCs anteriores
- [Link para a pasta](./UC9)

### 📂 UC10 - Desenvolvimento de Software
- Lógica de programação
- JavaScript básico
- Desenvolvimento web
- Ferramentas: VSCode
- [Link para a pasta](./UC10)

### 📂 UC11 - Desenvolvimento Desktop
- Interface gráfica
- Prototipação
- Design de aplicações
- Ferramentas: VSCode, Figma
- [Link para a pasta](./UC11)

### 📂 UC12 - Desenvolvimento Web
- HTML5, CSS3, JavaScript
- Design responsivo
- Front-end
- Ferramentas: VSCode
- [Link para a pasta](./UC12)

### 📂 UC15 - Design para Web
- UX/UI Design
- Layout responsivo
- Design moderno
- Ferramentas: VSCode
- [Link para a pasta](./UC15)

### 📂 UC18 - Ética e Direito Digital
- Legislação digital
- Privacidade e segurança
- Cidadania digital
- Ferramentas: Navegador
- [Link para a pasta](./UC18)

### 📂 UC19 - Empreendedorismo Tecnológico
- Modelo de negócios
- Marketing digital
- Pitch e apresentação
- Ferramentas: Navegador
- [Link para a pasta](./UC19)

### 📂 UC22 - Ciência de Dados
- Análise de dados
- Visualização
- Interpretação
- Ferramentas: Excel
- [Link para a pasta](./UC22)

### 📂 UC23 - Internet das Coisas
- Fundamentos IoT
- Automação
- Projetos práticos
- Ferramentas: Packet Tracer
- [Link para a pasta](./UC23)

## 💻 Como Usar Este Repositório

1. Cada pasta de UC contém:
   - README específico com instruções
   - Material teórico
   - Atividades práticas
   - Recursos necessários

2. Para começar:
   ```bash
   git clone https://github.com/seu-usuario/nome-do-repo
   cd nome-do-repo
   ```

## 📝 Instruções Gerais

1. **Prazos:**
   - Parte Teórica: 15 dias
   - Parte Prática: 30 dias

2. **Entrega:**
   - Teoria: Formato PDF/PPT
   - Prática: Arquivos específicos de cada UC

3. **Avaliação:**
   - Teoria: 40% da nota
   - Prática: 60% da nota
   - Pontos extras disponíveis

## 🛠️ Ferramentas Necessárias

- VSCode
- Cisco Packet Tracer
- Hyper-V
- Microsoft Excel
- Navegador moderno
- Bloco de notas
- Figma (conta educacional)

## 📚 Recursos Adicionais

- W3Schools: https://www.w3schools.com
- MDN Web Docs: https://developer.mozilla.org
- Cisco Learning Network: https://learningnetwork.cisco.com
- Debian Documentation: https://www.debian.org/doc/

## 🤝 Suporte

- Dúvidas por email
- Atendimento em aula
- Material de apoio disponível
- Consulta à internet permitida

## 📜 Licença

Este material é destinado exclusivamente para fins educacionais no âmbito do curso Técnico em Informática.

## ✉️ Contato

- Professor: André Ricardo
- Email: <andre.ricardo@colegiovictorino.com.br>
